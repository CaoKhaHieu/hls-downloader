...
import indexDBService from "./IndexedDBService.js";
...
class DownloadManager {
  ...
  async start(url) {
    ...
    await indexDBService.save(idVideoIDB, this.infoVideoDownload[idVideoIDB]);

    // check if the number of videos being downloaded is greater than the maximum
    if (this.countDownload >= MAX_VIDEOS_DOWNLOADING) {
      this.videosWaiting = [...this.videosWaiting, this.infoVideoDownload[idVideoIDB]];
      return;
    }
